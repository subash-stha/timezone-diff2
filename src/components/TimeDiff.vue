  ...
  <div class="hello">
    ...
    <div class="row">
      <div class="col-md-6" style="float: left; width: 50%;">
        <select class="form-control" style="padding-left: 15px; width: 50%; height: 55px;" @change="changeFirstTimeZone($event)">
          <option value="" selected disabled>Choose first time zone</option>
          <option v-for="zone in zones" :value="zone.zoneName" :key="zone.gmtOffset">{{ zone.zoneName }}</option>
        </select>
        <br><br>
        <p><span>Selected First Time Zone: {{ selectedZoneFirst  }}</span></p>
      </div>
      <div class="col-md-6" style="float: left; width: 50%;">
        <select class="form-control" style="padding-left: 15px; width: 50%; height: 55px;" @change="changeSecondTimeZone($event)">
          <option value="" selected disabled>Choose second time zone</option>
          <option v-for="zone in zones" :value="zone.zoneName" :key="zone.gmtOffset">{{ zone.zoneName }}</option>
        </select>
        <br><br>
        <p><span>Selected Second Time Zone: {{ selectedZoneSecond  }}</span></p>
      </div>
    </div>
    <div class="row">
      <button style="background: coral; height: 40px; border-radius: 10%;" @click="checkDifference">Check Time Difference</button>
    </div>
    <p><span>Time Difference between two zones: {{ finalDiff  }}</span></p>
